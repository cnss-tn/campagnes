document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderUsersPage(page) {
        ...
        slice.forEach((row) => {
            ...
            tr.innerHTML = `
                ...
                <td><span class="grade-circle">${row[3] || ''}</span></td>
    ...
    inputImportXlsx?.addEventListener('change', async (e) => {
        ...
        // Expected (import): Matricule, Nom AR, Nom FR, Grade, Code Bureau, Type, Pw - Bureau name not included, only Code number; tolerant on order/spelling
        const findIdx = (names) => {
        ...
        for (let r = 1; r < aoa.length; r++) {
            ...
            if (hasPw && !pw) {
                errors.push(`سطر ${r + 1}: Pw (كلمة المرور) فارغة - يجب ملؤها بعد Type`);
                continue;
            }
        ...
        try {
            // 1) Delete existing (keep self)
            for (const row of [...usersAllRows]) {
                const mat = String(row[0] || '').trim();
                if (!mat || mat === myMatricule) continue;
                try {
                    const res = await postAction('adminDeleteUser', { matricule: mat });
            ...
            for (const u of toImport) {
                ...
                if (isSelf) payload.isEdit = true;
                try {
                    const res = await postAction('adminSaveUser', payload);
                    if (res && res.ok) okCount++;
                    else {
                        // Try as update if create failed due to duplicate
                        payload.isEdit = true;
                        const res2 = await postAction('adminSaveUser', payload);
                        if (res2 && res2.ok) okCount++; else failCount++;
                    }
                } catch { failCount++; }
            }
            alert(`تم: ${okCount} مستخدم(ين) مستورد(ين)${failCount ? `, فشل: ${failCount}` : ''}`);
            await loadUsers();
        } finally {
